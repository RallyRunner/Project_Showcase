...
def solve(section_data, ta_data):
    # clear best_solutions & solutions.dat for pickling instances
    for filename in glob.glob(f"{path}/results/best_solutions*.csv"):
        file_remove = Path(path / f"results/{filename}")
        file_remove.unlink(missing_ok=True)
    solutions_remove = Path(path / "results/solutions.dat")
    solutions_remove.unlink(missing_ok=True)
...
st.dataframe(sections_example)
st.subheader("tas.csv Example")
st.dataframe(tas_example)
